...
export function renderMarkdownToHtml(markdown: string, r2PublicUrl?: string) {
	...
	const sanitizeConfig: any = {
		...
		ADD_ATTR: ['target', 'rel', 'autoplay', 'muted', 'playsinline', 'preload', 'data-fancybox', 'data-caption', 'data-media-id'],
		ADD_CLASSES: ['md-indent-paragraph', 'media-inline']
	};
	html = DOMPurify.sanitize(html, sanitizeConfig) as unknown as string;
	// 给视频加内联样式居中
