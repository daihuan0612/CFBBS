		...
		const jsonResponse = (data: any, status = 200, cacheOverride?: string) => {
			...
			if (cacheOverride) {
				...
				const cacheablePaths: [string, string][] = [
					['/api/posts', 'public, max-age=0, s-maxage=3600, stale-while-revalidate=86400'],
...
// Check file size (2MB = 2 * 1024 * 1024 bytes)
			...
			try {
				...
				if (avatar_url !== undefined && avatar_url !== currentUser.avatar_url) {
					...
					if (currentUser.avatar_url) {
						ctx.waitUntil(deleteImage(env as unknown as S3Env, currentUser.avatar_url, user_id).catch(() => {}));
					}
		...
		if (url.pathname === '/api/user/delete' && method === 'POST') {
			try {
				...
				// 1. Delete images (Avatar + Post images + Post thumbnails)
				const posts: any = await env.cforum_db.prepare('SELECT content, thumbnail_url FROM posts WHERE author_id = ?').bind(user_id).all();
				...
				if (posts.results) {
					for (const post of posts.results) {
						...
						if (post.thumbnail_url) {
							deletionPromises.push(deleteImage(env as unknown as S3Env, post.thumbnail_url as string, user_id));
						}
					}
				}
...
const user = await env.cforum_db.prepare('SELECT * FROM users WHERE email_change_token = ?').bind(token).first<DBUser>();
		...
		if (url.pathname.match(/^\/api\/admin\/users\/\d+\/update$/) && method === 'POST') {
			...
			try {
				...
				if (avatar_url !== undefined) {
					// 先查旧头像 URL 用于清理 R2
					const oldUser = await env.cforum_db.prepare('SELECT avatar_url FROM users WHERE id = ?').bind(id).first<{avatar_url: string | null}>();
					const oldAvatar = oldUser?.avatar_url || null;
					...
					if (oldAvatar && oldAvatar !== avatar_url) {
						ctx.waitUntil(deleteImage(env as unknown as S3Env, oldAvatar, parseInt(id)).catch(() => {}));
					}
		...
		if (url.pathname.match(/^\/api\/admin\/users\/\d+$/) && method === 'DELETE') {
			...
			try {
				...
				const posts = await env.cforum_db.prepare('SELECT content, thumbnail_url FROM posts WHERE author_id = ?').bind(id).all();
				...
				if (posts.results) {
					for (const post of posts.results) {
						...
						if ((post as any).thumbnail_url) {
							deletionPromises.push(deleteImage(env as unknown as S3Env, (post as any).thumbnail_url as string, id));
						}
					}
				}
				if (deletionPromises.length > 0) {
		...
		if (url.pathname.match(/^\/api\/admin\/posts\/\d+$/) && method === 'DELETE') {
			...
			try {
				...
				const post = await env.cforum_db.prepare('SELECT content, author_id, thumbnail_url FROM posts WHERE id = ?').bind(id).first();
				if (post) {
					const imageUrls = extractImageUrls(post.content as string);
					...
					// 删除 R2 缩略图
					if (post.thumbnail_url) {
						ctx.waitUntil(deleteImage(env as unknown as S3Env, post.thumbnail_url as string, post.author_id as number).catch(() => {}));
					}
				}
		...
		if (url.pathname === '/api/admin/cleanup/analyze' && method === 'GET') {
			try {
				...
				// Posts thumbnail_url (VideoThumbnail 永久缓存)
				const postThumbs = await env.cforum_db.prepare('SELECT thumbnail_url FROM posts WHERE thumbnail_url IS NOT NULL').all();
				for (const p of postThumbs.results) {
					const key = p.thumbnail_url ? getKeyFromUrl(env as unknown as S3Env, p.thumbnail_url as string) : null;
					if (key) usedKeys.add(key);
				}

				// media_files 中的缩略图和原始 URL
				const mediaFiles = await env.cforum_db.prepare('SELECT thumbnail, url FROM media_files').all();
				for (const m of mediaFiles.results) {
					const mRow = m as { thumbnail?: string | null; url?: string | null };
					for (const field of [mRow.thumbnail, mRow.url]) {
						const key = field ? getKeyFromUrl(env as unknown as S3Env, field) : null;
						if (key) usedKeys.add(key);
					}
				}
		...
		if (url.pathname === '/api/posts' && method === 'GET') {
			try {
				...
				let query = `SELECT
                        ...
                        COALESCE(posts.thumbnail_url, pmt.thumb, pmt.media_url) as thumbnail
                     FROM posts
                     ...
                     LEFT JOIN (SELECT pm.post_id, mf.thumbnail as thumb, mf.url as media_url FROM post_media pm JOIN media_files mf ON pm.media_id = mf.id WHERE mf.media_type IN ('image','video') GROUP BY pm.post_id) pmt ON pmt.post_id = posts.id`;
				...
				resp.headers.set('CDN-Cache-Control', 'public, max-age=3600');
				return resp;
			} catch (e) {
				return handleError(e);
			}
		...
		if (url.pathname.match(/^\/api\/posts\/\d+$/) && method === 'GET') {
			...
			// 无 user_id → 公开内容，CDN 长缓存；带 user_id → 个性化（含 liked 状态），不缓存
			if (!userId) {
				resp.headers.set('Cache-Control', 'public, max-age=0, s-maxage=86400, stale-while-revalidate=604800');
				resp.headers.set('CDN-Cache-Control', 'public, max-age=86400');
			} else {
		...
		if (url.pathname.match(/^\/api\/posts\/\d+$/) && method === 'DELETE') {
			...
			try {
				...
				// Check ownership
				const post = await env.cforum_db.prepare('SELECT author_id, content, thumbnail_url FROM posts WHERE id = ?').bind(id).first();
				if (!post) return jsonResponse({ error: 'Post not found' }, 404);
				...
				// 删除 R2 缩略图
				if (post.thumbnail_url) {
					ctx.waitUntil(deleteImage(env as unknown as S3Env, post.thumbnail_url as string, userPayload.id).catch(() => {}));
				}
		...
		if (url.pathname.match(/^\/api\/posts\/\d+\/comments$/) && method === 'GET') {
			...
			try {
				...
				const resp = jsonResponse(results);
				resp.headers.set('Cache-Control', 'public, max-age=10, s-maxage=60, stale-while-revalidate=300');
				resp.headers.set('CDN-Cache-Control', 'public, max-age=60');
				return resp;
			} catch (e) {
				return handleError(e);
			}
		...
		if (url.pathname === '/api/video-proxy' && (method === 'GET' || method === 'HEAD')) {
			try {
				...
				// 视频文件内容不变，长缓存减少回源
				proxyHeaders.set('Cache-Control', 'public, max-age=0, s-maxage=604800, stale-while-revalidate=86400');
				proxyHeaders.set('CDN-Cache-Control', 'public, max-age=604800');
